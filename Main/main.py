...
def update_gui_labels():
    ...
    if response and "main" in response:
        ...
        pressure_data = ck.CTkLabel(frame_bottom_right, text=(pressure), font=("Veranda", 55))
        pressure_data.place(x=60,y=110)
...
#Feels LIKE ==============================================================================
        ...
        feels_like = str(kelvin_to_Fahrenheit_Celsius(feels_like_kelvin))

        if "." in (feels_like) and "(" in feels_like:
            x = feels_like.index(".")
            y = feels_like.index("(")
            feels_like = feels_like[y+1:x]
            feels_like = int(feels_like)
            if feels_like < 20:
                my_img1 = ck.CTkImage(dark_image=Image.open("Cold.png"),size=(200,200),)
            elif 20 < feels_like < 50:
                my_img1 = ck.CTkImage(dark_image=Image.open("Chilly.png"),size=(200,200),)
            elif 50 < feels_like < 75:
                my_img1 = ck.CTkImage(dark_image=Image.open("Perfect.png"),size=(200,200),)
            elif 75 < feels_like < 90:
                my_img1 = ck.CTkImage(dark_image=Image.open("Warm.png"),size=(200,200),)
            elif feels_like > 90:
                my_img1 = ck.CTkImage(dark_image=Image.open("Hot.png"),size=(200,200),)

            temp_meter = ck.CTkLabel(frame_bottom_left,image=my_img1,text="")
            temp_meter.place(x=25,y=40)
            feels_like_spot = ck.CTkLabel(frame_bottom_left, text=f"{feels_like}", font=("Verdana", 55))
            feels_like_spot.place(x=95,y=100)

        else:
            feels_like_spot = ck.CTkLabel(frame_bottom_left, text=f"{feels_like}", font=("Verdana", 55))
            feels_like_spot.place(x=100,y=100)
        ...
        blackline_pic = ck.CTkImage(dark_image=Image.open("Black_Line.jpeg"),size=(5,170),)
        blackline = ck.CTkLabel(frame_bottom_middle,image=blackline_pic,text="")
        blackline.place(x = 131, y=112)
